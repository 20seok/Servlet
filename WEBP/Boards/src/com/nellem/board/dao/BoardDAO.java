...
//게시글 상세 보기	
	public BoardDTO view(int no) {
		String sql ="select no, title, writer, content, regdate from board where no=?";
		BoardDTO dto = new BoardDTO();
		try {
			con = ds.getConnection();
			pstmt = con.prepareStatement(sql);
...
// 게시글 추가하기
	public boolean insert(BoardDTO dto) {	//DB에 저장이 잘되면 true, 잘안되었으면  false를 반환
		String sql = "insert into board(no, title, writer, content, regdate) values(SEQ_BOARD_NEXT_NO.nextval, ?,?,?, SYSDATE)"; 
		boolean check = false;
		try {  
